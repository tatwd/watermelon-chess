...
class Board : public QMainWindow
{
  ...
  public:
	...
	/*绘制棋盘及棋子*/
	void init(); //初始化函数
	QPoint polarCoordToXY(QPoint, double angle,
						  double r); //极坐标转化成棋子的像素坐标
	...
	bool getPolarCoord(QPoint pt, int &angle,
					   int &r); //像素坐标转成棋子的极点坐标并判断是否有子
	...
	void back(Step *step);
	void backStep(); //悔一步
	bool isKillIdOfStep(int id,
						QVector<Step *> steps); //判断死子是否已经被棋子杀死
